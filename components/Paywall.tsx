...
import { Lock, ShieldCheck, ArrowRight, Zap, Eye } from 'lucide-react'

interface PaywallProps {
    tallerId: string;
    email: string;
    fechaVencimiento: string;
    onIgnorar: () => void; // 🔥 Nueva función para cerrar el paywall
}

export default function Paywall({ tallerId, email, fechaVencimiento, onIgnorar }: PaywallProps) {
    ...
    return (
        // 🔥 Cambiamos <main> por un <div> fijo sobre toda la app con blur
        <div className="fixed inset-0 z-[999] bg-slate-950/60 backdrop-blur-md flex flex-col items-center justify-center p-4">
            ...
            <div className="w-full max-w-lg bg-slate-900/90 backdrop-blur-xl p-8 sm:p-12 rounded-[40px] border border-slate-800 shadow-2xl relative z-10 text-center">
                ...
                <p className="text-sm font-bold text-slate-400 mb-8 leading-relaxed">
                    ...
                    Renueva tu acceso ahora para seguir creando órdenes y gestionando ingresos.
                </p>
                ...
                <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-5 mb-8 text-left space-y-3">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-4">
                        <Zap size={14} className="text-emerald-500"/> Al renovar recuperas
                    </h3>
                    <div className="flex items-center gap-3 text-xs font-bold text-slate-300">
                        <ShieldCheck size={16} className="text-emerald-500" /> Creación de órdenes ilimitadas
                    </div>
                    <div className="flex items-center gap-3 text-xs font-bold text-slate-300">
                        <ShieldCheck size={16} className="text-emerald-500" /> Actas de recepción y alertas 3D
                    </div>
                    <div className="flex items-center gap-3 text-xs font-bold text-slate-300">
                ...
                >
                    {cargandoPago ? 'Conectando con pasarela...' : <>Pagar Mensualidad ($24.990) <ArrowRight size={16} /></>}
                </button>

                {/* 🔥 BOTÓN SECUNDARIO: SOLO LECTURA */}
                <button 
                    onClick={onIgnorar}
                    className="w-full mt-4 bg-transparent border border-slate-700 text-slate-400 py-4 px-6 rounded-full font-black uppercase tracking-widest text-xs hover:bg-slate-800 hover:text-slate-200 transition-all flex items-center justify-center gap-2"
                >
                    <Eye size={16} /> Seguir usando (Solo Lectura)
                </button>

                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-6 flex items-center justify-center gap-1">
                    <ShieldCheck size={10} /> Pago seguro procesado por Flow Chile
                </p>
            </div>
        </div>
    )

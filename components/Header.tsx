...
export default function Header({
    ...
    onOpenConfiguracion,
    onOpenMarketing,
    mecanicoActivo // 🔥 Recibimos al mecánico para aplicar la censura
}: any) {
    ...
    useEffect(() => {
        const fetchDatosHeader = async () => {
            // Si es un mecánico, no buscamos logo ni días de prueba en Auth, 
            // asumimos los datos base o los que vengan de su taller (puedes expandir esto luego si quieres que vean el logo)
    ...
    // Función para que el mecánico cierre su sesión local
    ...
    return (
        <div className="flex flex-col w-full mb-6 z-10 relative">
            ...
            {/* 🔥 BANNER DE PRUEBA (Solo para el dueño) */}
            ...
            <header className="bg-slate-900/50 backdrop-blur-md border border-slate-800 p-4 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0 rounded-3xl shadow-2xl w-full">
                ...
                <div className="flex items-center justify-center flex-wrap gap-2 md:gap-3 w-full md:w-auto">
                    ...
                    {/* Botón Scanner IA: Disponible para TODOS */}
                    ...
                    {/* 🔒 BOTONES CENSURADOS: SOLO PARA EL DUEÑO */}
                    {!mecanicoActivo && (
                        <>
                    ...
                    {/* 🔥 BOTÓN DE SALIDA PARA EL MECÁNICO */}
                    {mecanicoActivo && (
                        <>
                            <div className="w-px h-8 bg-slate-800 mx-1 hidden md:block"></div>
                            <button onClick={cerrarSesionMecanico} className="group flex items-center gap-2.5 bg-red-600/10 border border-red-600/30 hover:bg-red-600 hover:border-red-500 px-4 py-2.5 rounded-xl text-red-500 hover:text-white transition-all shadow-lg hover:scale-105 shrink-0" title="Cerrar Turno">
                                <LogOut size={16} />
                                <span className="text-[10px] font-black uppercase tracking-widest">Cerrar Turno</span>
                            </button>
                        </>
                    )}

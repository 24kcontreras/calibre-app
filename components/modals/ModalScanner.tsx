...
export default function ModalScanner({
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-[200]">
      <div className="bg-slate-900 p-8 rounded-[40px] shadow-2xl max-w-lg w-full border border-slate-800 relative flex flex-col max-h-[90vh]">
        
        <button onClick={onClose} className="absolute top-6 right-6 text-slate-500 hover:text-emerald-400 transition-colors z-10">
          <X size={24} />
        </button>
        ...
        <div className="flex bg-slate-950 p-1 rounded-2xl mb-6">
          ...
          >
            <Activity size={16} /> Scanner OBD2
          </button>
          <button 
            onClick={() => { setModoActivo('manual'); setCodigoScanner(''); }}
            className={`flex-1 py-3 flex items-center justify-center gap-2 font-black text-xs uppercase tracking-widest rounded-xl transition-all ${modoActivo === 'manual' ? 'bg-blue-600 text-slate-950 shadow-lg' : 'text-slate-500 hover:text-blue-400'}`}
          >
        ...
        <form onSubmit={handleConsultar} className="space-y-4 shrink-0">
          <input 
            value={vehiculoScanner} 
            onChange={e => setVehiculoScanner(e.target.value)} 
            required
            placeholder="Vehículo (Ej: Kia Sportage 2008 CRDI)" 
            className={`w-full p-4 rounded-2xl border bg-slate-800 font-bold text-slate-100 outline-none transition-all ${modoActivo === 'scanner' ? 'border-slate-700 focus:border-emerald-500' : 'border-slate-700 focus:border-blue-500'}`} 
          />
          ...
          <div className="flex gap-2">
            <input 
              ...
              className={`flex-1 p-4 rounded-2xl border-2 bg-slate-800 font-black outline-none transition-all ${modoActivo === 'scanner' ? 'border-emerald-900/50 text-emerald-400 focus:border-emerald-500 placeholder:text-emerald-900/50 uppercase' : 'border-blue-900/50 text-blue-400 focus:border-blue-500 placeholder:text-blue-900/50'}`} 
            />
            <button type="submit" disabled={cargandoScanner} className={`px-6 rounded-2xl font-black text-slate-950 transition-all disabled:opacity-50 ${modoActivo === 'scanner' ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-blue-600 hover:bg-blue-500'}`}>
              <Search size={20} />
            </button>
          </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {sugerenciasManual.map((sug, idx) => (
                <button 
                  ...
                  className="text-[10px] font-bold bg-blue-950/50 text-blue-400 border border-blue-900/50 px-3 py-1.5 rounded-lg hover:bg-blue-900 hover:text-blue-300 transition-colors"
                >
                  {sug}
                </button>
              ))}
        ...
        <div className="mt-6 flex-1 overflow-y-auto custom-scrollbar-dark rounded-2xl bg-slate-950/80 border border-slate-800 p-6 relative">
          {cargandoScanner ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-600 text-xs font-bold text-center italic px-4">
